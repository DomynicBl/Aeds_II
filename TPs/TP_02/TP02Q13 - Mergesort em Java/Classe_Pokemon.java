...
class MetodosDeOrdenacao{
    ...
    public static void mergeSort(ArrayList<Pokemon> pokedex) {
        if (pokedex.size() < 2) return; // Se a lista tiver 1 ou nenhum elemento, não precisa ordenar

        int meio = pokedex.size() / 2; // Elemento do meio da lista
        ArrayList<Pokemon> esq = new ArrayList<>(pokedex.subList(0, meio)); // Sublista esquerda
        ArrayList<Pokemon> dir = new ArrayList<>(pokedex.subList(meio, pokedex.size())); // Sublista direita
        ...
        intercalar(pokedex, esq, dir);
    }

    private static void intercalar(ArrayList<Pokemon> pokedex, ArrayList<Pokemon> esq, ArrayList<Pokemon> dir) {
        int i = 0, j = 0, k = 0; // Índices para a sublista esq, dir e lista original

        while (i < esq.size() && j < dir.size()) {
            // Comparar tipos dos Pokémons
            if (esq.get(i).getTypes().procurar(0).compareTo(dir.get(j).getTypes().procurar(0)) < 0 || ( // Valida de o tipo da sublista esquerda é maior
                esq.get(i).getTypes().procurar(0).equals(dir.get(j).getTypes().procurar(0)) &&  // Se os tipos forem iguais
                esq.get(i).getName().compareTo(dir.get(j).getName()) < 0)) { // Comparar nomes
                pokedex.set(k++, esq.get(i++)); // Adiciona elemento da sublista esquerda
                comparacoes+=2; // Incrementar comparações ao comparar elementos
            } else { // Se não comprir a condição acima, é porque o elemento da sublista direita é maior
                pokedex.set(k++, dir.get(j++)); // Adiciona elemento da sublista direita
            }
            
            movimentacoes++; // Incrementar movimentação ao adicionar elemento na lista
        }

        // Copiar os elementos restantes da sublista esquerda, se houver
        while (i < esq.size()) {
            pokedex.set(k++, esq.get(i++));
            movimentacoes++;
        }

        // Copiar os elementos restantes da sublista direita, se houver
        while (j < dir.size()) {
            pokedex.set(k++, dir.get(j++));
            movimentacoes++;
        }
    }
} 
...
public class Classe_Pokemon{
    ...
    public static void main(String[] args){
        ...
        MetodosDeOrdenacao.mergeSort(pokedex);
        ...
        try {
            ...
            File arquivoLog = new File("matrícula_mergesort.txt"); // Cria o arquivo de log
