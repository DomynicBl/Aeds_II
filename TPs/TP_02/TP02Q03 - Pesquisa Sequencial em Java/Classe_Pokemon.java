...
class MetodosDePesquisa{
    public static int comparacoes = 0; // Contador de comparações

    public static void pesquisaSequencial(String search_key, ArrayList<Pokemon> pokedex) {
        boolean encontrado = false;
        int i = 0;
    
        while (i < pokedex.size() && !encontrado) { // Enquanto não encontrar o pokemon e não chegar ao final da pokedex
            Pokemon pokemon = pokedex.get(i); // Obtém o pokemon na posição "i"
            comparacoes++; // Incrementa o contador de comparações
            if (pokemon.getName().equals(search_key)) {
                encontrado = true;
            }
            i++;
        }
    ...
    public static void main(String[] args){
        ...
        while(abort == false){
            ...
            if(input.equalsIgnoreCase("FIM")){
                ...
                MetodosDePesquisa.pesquisaSequencial(input, pokedex);
            }
        }
        ...
        try {
            ...
            File arquivoLog = new File("matricula_sequencial.txt"); // Cria o arquivo de log
            try (PrintWriter writer = new PrintWriter(arquivoLog, StandardCharsets.UTF_8)) { // Cria um objeto para escrever no arquivo
                writer.printf("Matrícula:%s\tExecução(ms):%d\tComparações:%d%n", matricula, tempoExecucao, MetodosDePesquisa.comparacoes);// Escreve no arquivo
            }
