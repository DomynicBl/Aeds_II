...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_radixsort.txt", "w");
    if (arquivoLog == NULL) {
...
void radcountingSort(Pokedex *pokedex, int exp, int maxLen) {
    Pokedex output;
    output.count = pokedex->count;
    
    int count[MAX_CHAR] = {0};
    
    // Contar ocorrências de cada caractere na posição 'exp'
    for (int i = 0; i < pokedex->count; i++) {
        char c = pokedex->pokemons[i].abilities.elementos[0][exp]; // Considerando a primeira habilidade
        count[(unsigned char)c]++;
    }
    
    // Modificar count[] para conter as posições finais
    for (int i = 1; i < MAX_CHAR; i++) {
        count[i] += count[i - 1];
    }
    
    // Construir a saída
    for (int i = pokedex->count - 1; i >= 0; i--) {
        char c = pokedex->pokemons[i].abilities.elementos[0][exp];
        output.pokemons[count[(unsigned char)c] - 1] = pokedex->pokemons[i];
        count[(unsigned char)c]--;
    }
    
    // Copiar de volta para a pokedex original
    for (int i = 0; i < pokedex->count; i++) {
        pokedex->pokemons[i] = output.pokemons[i];
        movimentacoes++;
    }
}

void radixSort(Pokedex *pokedex) { // Função principal do Radix Sort
    int maxLen = 0;
    Pokemon temp;
    
    // Encontra o comprimento máximo das habilidades
    for (int i = 0; i < pokedex->count; i++) {
        int len = strlen(pokedex->pokemons[i].abilities.elementos[0]); // Considera a primeira habilidade
        comparacoes++;
        if (len > maxLen) {
            maxLen = len;
        }
    ...
    // Aplica Counting Sort para cada posição da string (da direita para a esquerda)
    for (int exp = maxLen - 1; exp >= 0; exp--) {
        radcountingSort(pokedex, exp, maxLen);
    }
    
    // Em caso de empate nas habilidades, ordena por nome
    for (int i = 0; i < pokedex->count - 1; i++) {
        if (strcmp(pokedex->pokemons[i].abilities.elementos[0], pokedex->pokemons[i + 1].abilities.elementos[0]) == 0) {
            comparacoes++;
            if (strcmp(pokedex->pokemons[i].name, pokedex->pokemons[i + 1].name) > 0) { // Em caso de empate, compara por nome
                temp = pokedex->pokemons[i];
                pokedex->pokemons[i] = pokedex->pokemons[i + 1];
                pokedex->pokemons[i + 1] = temp;
...
int main() {
    ...
    // Segunda parte: Ordenação por Radix Sort
    radixSort(&pokedex); // Ordena a Pokedex
