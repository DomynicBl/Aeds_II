...
class MetodosDeOrdenacao{
    ...
    public static void Insercao(ArrayList<Pokemon> pokedex) {
        for (int i = 0; i < pokedex.size(); i++) {
            Pokemon temp = pokedex.get(i);
            int j = i - 1;

            while((j >= 0) && (Date.Comparar(pokedex.get(j).getDate(), temp.getDate()) == 1)) {
                comparacoes++;
...
public class Classe_Pokemon{
    ...
    public static void main(String[] args){
        ...
        MetodosDeOrdenacao.Insercao(pokedex);
        ...
        try {
            ...
            File arquivoLog = new File("matricula_insercao.txt"); // Cria o arquivo de log
