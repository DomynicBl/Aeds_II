...
class MetodosDeOrdenacao {
    ...
    public static void Heapsort(ArrayList<Pokemon> pokedex) {

        // Construir o heap
        for (int tamHeap = 2; tamHeap <= pokedex.size(); tamHeap++) { // Inicia em 2 para evitar a divisão por 0
            construir(pokedex, tamHeap);
        }

        // Ordenar o heap
        int tamHeap = pokedex.size();
        while (tamHeap > 1) { // Enquanto houver elementos no heap
            swap(pokedex, 1, tamHeap--); // Trocar elementos
            reconstruir(pokedex, tamHeap); // Reconstruir o heap
        }
    }

    // Função para construir o heap
    private static void construir(ArrayList<Pokemon> pokedex, int tamHeap) {
        for (int i = tamHeap; i > 1; i /= 2) { // Inicia em tamHeap e divide por 2
            if (comparePokemons(pokedex.get(i - 1), pokedex.get((i / 2) - 1)) > 0) {//Compara os elementos
                swap(pokedex, i, i / 2);
            }
        }
    }
    ...
    private static void reconstruir(ArrayList<Pokemon> pokedex, int tamHeap) {
        int i = 1;
        while (i <= tamHeap / 2) {//Enquanto i for menor ou igual a metade do heap
            int filho = getMaiorFilho(pokedex, i, tamHeap);

            if (comparePokemons(pokedex.get(i - 1), pokedex.get(filho - 1)) < 0) {
    ...
    private static int getMaiorFilho(ArrayList<Pokemon> pokedex, int i, int tamHeap) {
        int filho;

        if (2 * i == tamHeap || comparePokemons(pokedex.get(2 * i - 1), pokedex.get(2 * i)) > 0) {
            filho = 2 * i;
        } else {
            filho = 2 * i + 1;
        }
    ...
    // Função para comparar dois Pokémons levando em conta altura e nome
    private static int comparePokemons(Pokemon p1, Pokemon p2) {
        ...
        if (p1.getHeight() > p2.getHeight()) {
            ...
            return -1;
        } else { // Se as alturas forem iguais, o criterio de desempate é o nome
    ...
    private static void swap(ArrayList<Pokemon> pokedex, int i, int j) {
        ...
        pokedex.set(i - 1, pokedex.get(j - 1));
...
public class Classe_Pokemon{
    ...
    public static void main(String[] args){
        ...
        MetodosDeOrdenacao.Heapsort(pokedex);
        ...
        try {
            ...
            File arquivoLog = new File("matricula_heapsort.txt"); // Cria o arquivo de log
