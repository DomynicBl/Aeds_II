...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_shellsort.txt", "w");
    if (arquivoLog == NULL) {
...
void shellSort(Pokedex *pokedex) {
    int n = pokedex->count;
    int gap, i, j;
    Pokemon temp;
    
    for (gap = n / 2; gap > 0; gap /= 2) {  // Define o gap inicial e o reduz progressivamente
        for (i = gap; i < n; i++) {
            temp = pokedex->pokemons[i];
            j = i;
            
            while (j >= gap && (pokedex->pokemons[j - gap].weight > temp.weight || ( // Ordenação pelo peso 
                                pokedex->pokemons[j - gap].weight == temp.weight && 
                                strcmp(pokedex->pokemons[j - gap].name, temp.name) > 0))) { // Em caso de empate, ordena pelo nome
                                
                pokedex->pokemons[j] = pokedex->pokemons[j - gap];
                j -= gap;
                comparacoes+=2;
                movimentacoes++;
            }
            
            pokedex->pokemons[j] = temp;
            movimentacoes++;
        }
    }
}
...
int main() {
    ...
    // Segunda parte: Ordenação por Shell Sort
    shellSort(&pokedex); // Ordena a Pokedex
