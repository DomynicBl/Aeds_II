...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_bolha.txt", "w");
    if (arquivoLog == NULL) {
...
void bubbleSort(Pokedex *pokedex) {
    int n = pokedex->count;
    int i, j;
    Pokemon temp;
    ...
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
            if (pokedex->pokemons[j].id > pokedex->pokemons[j + 1].id || ( // Compara pelo ID 
                pokedex->pokemons[j].id == pokedex->pokemons[j + 1].id && 
                strcmp(pokedex->pokemons[j].name, pokedex->pokemons[j + 1].name) > 0)) { // Em caso de empate, compara por nome
                comparacoes+=2; // Incrementa o contador de comparações
                
                temp = pokedex->pokemons[j];
                pokedex->pokemons[j] = pokedex->pokemons[j + 1];
                pokedex->pokemons[j + 1] = temp;
...
int main() {
    ...
    // Segunda parte: Ordenação por Bubble Sort
    bubbleSort(&pokedex); // Ordena a Pokedex
