...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_quicksort.txt", "w");
    if (arquivoLog == NULL) {
...
int compararPokemon(const Pokemon *p1, const Pokemon *p2) { // Função para comparar dois Pokémons por geração
    if (p1->generation < p2->generation) {
        ...
        comparacoes+=3;
        return strcmp(p1->name, p2->name); // Em caso de empate, compara por nome
    }
}

void quicksortRec(Pokemon *pokemons, int esq, int dir) { // Função recursiva para o QuickSort
    int i = esq, j = dir;
    Pokemon pivo = pokemons[(esq + dir) / 2]; // Escolhe o pivô como o elemento do meio
    
    while (i <= j) {
        while (compararPokemon(&pokemons[i], &pivo) < 0) i++; // Avança o indice até encontrar elemento maior ou igual ao pivô
        while (compararPokemon(&pokemons[j], &pivo) > 0) j--; // Retrocede o indice até encontrar elemento menor ou igual ao pivô
        
        if (i <= j) {
            swap(&pokemons[i], &pokemons[j]); // Faz a troca
            i++;
            j--;
        }
    }
    
    if (esq < j) quicksortRec(pokemons, esq, j); // Recursão na parte esquerda
    if (i < dir) quicksortRec(pokemons, i, dir); // Recursão na parte direita
}

void quickSort(Pokedex *pokedex) {
    quicksortRec(pokedex->pokemons, 0, pokedex->count - 1); // Aplica o QuickSort nos Pokémons da Pokedex
}
...
int main() {
    ...
    // Segunda parte: Ordenação por Quick Sort
    quickSort(&pokedex); // Ordena a Pokedex
