...
class MetodosDeOrdenacao{
    ...
    public static void ParcialQuickSort(ArrayList<Pokemon> pokedex, int esq, int dir, int k) {
        if (esq < dir && esq < k) { // Condiçaõ da oedewnação parcial (K)
            int pi = partition(pokedex, esq, dir);      // Obter o índice de partição
            ParcialQuickSort(pokedex, esq, pi - 1, k);  // Ordenar elementos antes da partição
            ParcialQuickSort(pokedex, pi + 1, dir, k);  // Ordenar elementos após a partição
        }
    }

    // Função para particionar o array
    public static int partition(ArrayList<Pokemon> pokedex, int esq, int dir) {
        Pokemon pivot = pokedex.get(dir); // Seleciona o pivô como o último elemento
        int i = (esq - 1); // Índice do menor elemento

        for (int j = esq; j < dir; j++) { 
            if (pokedex.get(j).getGeneration() < pivot.getGeneration() || ( // Compara a geração do Pokémon atual é menor que a do pivô
                pokedex.get(j).getGeneration() == pivot.getGeneration() &&  // Se as gerações são iguais
                pokedex.get(j).getName().compareTo(pivot.getName()) < 0)) { // Compara os nomes
                comparacoes+=2; // Conta a comparação
                i++; // Incrementa o índice do menor elemento

                Pokemon temp = pokedex.get(i);
                pokedex.set(i, pokedex.get(j));
                pokedex.set(j, temp);
                movimentacoes += 3; // Incrementa as movimentações
            }
        }

        Pokemon temp = pokedex.get(i + 1);
        pokedex.set(i + 1, pokedex.get(dir));
        pokedex.set(dir, temp);
        movimentacoes += 3; // Incrementa as movimentações
...
public class Classe_Pokemon{
    ...
    public static void main(String[] args){
        ...
        int k = 10;
        MetodosDeOrdenacao.ParcialQuickSort(pokedex, 0, pokedex.size() - 1, k);
        ...
        try {
            ...
            File arquivoLog = new File("matrícula_quicksortParcial.txt"); // Cria o arquivo de log
