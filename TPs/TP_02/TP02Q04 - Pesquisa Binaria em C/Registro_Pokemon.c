...
int compararNomes(const void *a, const void *b) { // Função para comparar os nomes dos Pokémons
    Pokemon *pokeA = (Pokemon *)a; // Converte o ponteiro para o tipo Pokemon
    Pokemon *pokeB = (Pokemon *)b; 
    return strcmp(pokeA->name, pokeB->name); 
}

void ordenarPokedex(Pokedex *pokedex) { // Usa a função qsort para ordenar a Pokedex
    qsort(pokedex->pokemons, pokedex->count, sizeof(Pokemon), compararNomes); 
}

//--------------------------------------------------------------------------------------------

void salvarLog(const char* matricula, long tempoExecucao, int comparacoes) {
    FILE *arquivoLog = fopen("matricula_binaria.txt", "w");
    if (arquivoLog == NULL) {
...
boolean pesquisaBinaria(Pokedex *pokedex, const char *nome) {
    int inicio = 0;
    int fim = pokedex->count - 1;

    while (inicio <= fim) {
        int meio = (inicio + fim) / 2;
        int comp = strcmp(pokedex->pokemons[meio].name, nome); // Compara o nome do Pokémon com o nome procurado
        comparacoes++;
...
int main() {
    ...
    // Segunda parte: pesquisa na Pokedex
    char name[MAX_TAM_NAME];
    abort = false;

    ordenarPokedex(&pokedex); // Ordena a Pokedex
    ...
    while (!abort) {
        ...
        if (strcmp(name, "FIM") == 0 || strcmp(name, "fim") == 0) {
            ...
            if (pesquisaBinaria(&pokedex, name)) {
                printf("SIM\n");
            } else {
                printf("NAO\n");
            }
