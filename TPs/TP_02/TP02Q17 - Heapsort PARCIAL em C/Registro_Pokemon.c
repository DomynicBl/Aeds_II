...
void imprimirPokedex(const Pokedex *pokedex, int k) {
    for (int i = 0; i < k; i++) {
...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_heapSortParcial.txt", "w");
    if (arquivoLog == NULL) {
...
void construir(Pokemon *pokemons, int tamHeap) {
    for (int i = tamHeap; i > 1 && (pokemons[i].height > pokemons[i / 2].height || // Verifica se o pai é menor que o filho
        (pokemons[i].height == pokemons[i / 2].height && strcmp(pokemons[i].name, pokemons[i / 2].name) > 0)); i /= 2) { // Em caso de empate, ordena por nome

        swap(&pokemons[i], &pokemons[i / 2]);
        comparacoes += 2;
    }
}

// Função para pegar o maior filho
int getMaiorFilho(Pokemon *pokemons, int i, int tamHeap) {
    int filho;
    if (2 * i == tamHeap || (pokemons[2 * i].height > pokemons[2 * i + 1].height || // Verifica se o filho da esquerda é maior que o da direita
        (pokemons[2 * i].height == pokemons[2 * i + 1].height && strcmp(pokemons[2 * i].name, pokemons[2 * i + 1].name) > 0))) { // Em caso de empate, ordena por nome
        filho = 2 * i;
    } else {
        filho = 2 * i + 1;
    }
...
void reconstruir(Pokemon *pokemons, int tamHeap) {
    int i = 1;
    while (i <= (tamHeap / 2)) {
        int filho = getMaiorFilho(pokemons, i, tamHeap);
        if (pokemons[i].height < pokemons[filho].height || // Verifica se o pai é menor que o filho
            (pokemons[i].height == pokemons[filho].height && strcmp(pokemons[i].name, pokemons[filho].name) < 0)) { // Em caso de empate, ordena por nome
            swap(&pokemons[i], &pokemons[filho]);
            i = filho;
        } else {
            i = tamHeap;
        }
...
// Função de Heapsort completa
void heapSort(Pokedex *pokedex, int k) {
    int n = pokedex->count;
    Pokemon tempArray[n + 1];

    // Copiar os Pokémons para o array temporário (ignorarando a posição 0)
    for (int i = 0; i < n; i++) {  
        tempArray[i + 1] = pokedex->pokemons[i];
    }

    // Construção do heap
    for (int tamHeap = 2; tamHeap <= n; tamHeap++) {  
        construir(tempArray, tamHeap);
    }

    // Ordenação propriamente dita
    int tamHeap = n;
    while (tamHeap > 1) {
        swap(&tempArray[1], &tempArray[tamHeap--]);
        reconstruir(tempArray, tamHeap);
    }

    // Copiar os Pokémons ordenados de volta para a Pokedex
    for (int i = 0; i < k; i++) {
        pokedex->pokemons[i] = tempArray[i + 1];
    }
}
...
int main() {
    ...
    // Segunda parte: Ordenação por Heap Sort
    int k = 10; // Número de Pokémons a serem ordenados
    heapSort(&pokedex, k); // Ordena a Pokedex
    imprimirPokedex(&pokedex, k); // Imprime a Pokedex
