...
void salvarLog(const char* matricula, long tempoExecucao, int comparacoes, int movimentacoes) {
    FILE *arquivoLog = fopen("matricula_insercaoParcial.txt", "w");
    if (arquivoLog == NULL) {
...
void insercaoParcial(Pokedex *pokedex, int k) {
    for (int i = 1; i < pokedex->count; i++) {
        Pokemon tmp = pokedex->pokemons[i];
        int j = (i<k)? i-1:k-1;

        while (j >= 0 && (pokedex->pokemons[j].captureDate.year > tmp.captureDate.year || ( // Compara pela data de captura
            pokedex->pokemons[j].captureDate.year == tmp.captureDate.year && pokedex->pokemons[j].captureDate.month > tmp.captureDate.month) || ( 
            pokedex->pokemons[j].captureDate.year == tmp.captureDate.year && pokedex->pokemons[j].captureDate.month == tmp.captureDate.month && 
            pokedex->pokemons[j].captureDate.day > tmp.captureDate.day) || ( pokedex->pokemons[j].captureDate.year == tmp.captureDate.year && 
            pokedex->pokemons[j].captureDate.month == tmp.captureDate.month && pokedex->pokemons[j].captureDate.day == tmp.captureDate.day && strcmp(
            pokedex->pokemons[j].name, tmp.name) > 0))) { // Em caso de empate, compara pelo nome
            comparacoes+=6;
                    
            pokedex->pokemons[j + 1] = pokedex->pokemons[j];  // Movimenta os Pokémons
            j--;

            movimentacoes++;
        }

        pokedex->pokemons[j + 1] = tmp;
        movimentacoes++;
    }
}
...
int main() {
    ...
    // Segunda parte: Ordenação por Inserção Parcial
    int k = 10; // Número de elementos a serem ordenados
    insercaoParcial(&pokedex, k); // Ordena a Pokedex
