...
class ListaDuplaFlexivel{
    ...
    public void inserir(Pokemon pokemon, int pos) throws 
    Exception {
        int tamanho = tamanho();
...
class MetodosDeOrdenacao {
    ...
    public static void QuickSort(ListaDuplaFlexivel pokedex) {
        if (pokedex != null && pokedex.primeiro != pokedex.ultimo) {
            quicksort(pokedex.primeiro.prox, pokedex.ultimo);
        }
    }
    ...
    private static CelulaDupla partition(CelulaDupla esq, CelulaDupla dir) {
        ...
        for (CelulaDupla j = esq; j != dir; j = j.prox) {
            ...
            if (compare(j.elemento, pivo) <= 0) {
                i = (i == null) ? esq : i.prox; // Incrementa `i` dentro da lista
                swap(i, j);
            }
        }

        i = (i == null) ? esq : i.prox; // Posiciona o pivô
        swap(i, dir);
        return i;
    }
...
public class Classe_Pokemon{
    public static void procurarPokemon(String id, ListaDuplaFlexivel pokedex) throws IOException {
    ...
    public static void main(String[] args){
        ListaDuplaFlexivel pokedex = new ListaDuplaFlexivel();
        ...
        // Segunda parte: Ordenação dos Pokémons (QuickSort)
        MetodosDeOrdenacao.QuickSort(pokedex);
    
        // Terceira parte: Mostrar Pokémons
        pokedex.mostrar();
        ...
        try {
            ...
            File arquivoLog = new File("matrícula_quicksort3.txt"); // Cria o arquivo de log
