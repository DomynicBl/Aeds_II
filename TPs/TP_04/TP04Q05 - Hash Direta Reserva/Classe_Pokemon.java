...
class HashReserva {
    ...
    private int tamTabela, tamReserva, tamTotal, reserva;
    private final Pokemon NULO = null;
    public int comparacoes = 0;

    public HashReserva() {
        this(21, 9); // Tamanhos padrão para a tabela principal e a reserva
    }
    ...
    public HashReserva(int tamTabela, int tamReserva) {
        ...
        this.tabela = new Pokemon[this.tamTotal];

        for (int i = 0; i < tamTotal; i++) { // Inicializa as posições da tabela como vazias
            tabela[i] = NULO; 
        }

        this.reserva = 0; // Inicializa o índice da área de reserva
    }

    private int hash(String name) { // Função de hash simples (soma dos valores ASCII dos caracteres)
        int ascii = 0;
    ...
    public boolean inserir(Pokemon pokemon) { //Insere um Pokémon na tabela hash
        if (pokemon != null) { // Verifica se o Pokémon é válido
            ...
            if (tabela[pos] == NULO) { // Verifica se a posição está vazia
                tabela[pos] = pokemon; // Insere na tabela principal
                return true;
            } else if (reserva < tamReserva) { // Verifica se ainda há espaço na área de reserva
                tabela[tamTabela + reserva] = pokemon; // Insere na área de reserva
                reserva++;
                return true;
            }
    ...
    public int pesquisar(String name) { // Pesquisa um Pokémon pelo nome na tabela hash
        ...
        if (tabela[pos] != null && tabela[pos].getName().equals(name)) {
            ...
            for (int i = 0; i < reserva; i++) { // Percorre a área de reserva
                if (tabela[tamTabela + i] != null && tabela[tamTabela + i].getName().equals(name)) { 
                    comparacoes+=2;
                    return tamTabela + i; // Retorna posição na área de reserva.
                }
            }
        }
...
public class Classe_Pokemon{
    public static void procurarPokemon(String id, HashReserva pokedex) throws IOException {
    ...
    public static void main(String[] args){
        HashReserva pokedex = new HashReserva();
        ...
        try {
            ...
            File arquivoLog = new File("matrícula_hashReserva.txt"); // Cria o arquivo de log
